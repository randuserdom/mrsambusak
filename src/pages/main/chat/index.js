...
export function Chat({ ws }) {
  ...
  const DeleteMessage = async (message_id) => {
    try {
      await axios.delete(`http://localhost:8000/message/${message_id}`);
      ws.send("");
    } catch (error) {
  ...
  const getCurrentUserDetails = useCallback(async () => {
    try {
      const response = await axios.get(
        `http://localhost:8000/user/${current_user_id}`
      );
      setUserDetails({
  ...
  return (
    <div className="container">
      ...
      <div className="d-flex">
        <div
          className="border mr-2 mt-4 overflow-auto"
          style={{ flex: "1", height: 490, borderRadius: 3 }}
        >
          <h4 style={{ marginLeft: 12 }}>Group Users</h4>
          {users.map(({ id, username, online }, index) => {
      ...
      <div
        ...
        style={{ width: "38.5%", marginTop: 20, borderRadius: 3 }}
      >
